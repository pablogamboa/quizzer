...
npx wrangler d1 execute quizzer-db --local --command="SELECT type, COUNT(*) FROM questions GROUP BY type"

# View categories
npx wrangler d1 execute quizzer-db --local --command="SELECT DISTINCT category FROM questions ORDER BY category"

# Get next question ID
npx wrangler d1 execute quizzer-db --local --command="SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM questions"
```
...
```bash
# Export questions
npx wrangler d1 execute quizzer-db --local --command="SELECT * FROM questions" > questions_backup.json
```
...
```bash
# View all themes
npx wrangler d1 execute quizzer-db --local --command="SELECT * FROM quiz_themes"

# View themes for a specific category
npx wrangler d1 execute quizzer-db --local --command="SELECT * FROM quiz_themes WHERE category='Geography'"

# Count questions per theme
npx wrangler d1 execute quizzer-db --local --command="SELECT theme, COUNT(*) as count FROM questions WHERE theme IS NOT NULL GROUP BY theme"

# View all themed questions
npx wrangler d1 execute quizzer-db --local --command="SELECT id, question, category, theme FROM questions WHERE theme IS NOT NULL"

# Disable a theme
npx wrangler d1 execute quizzer-db --local --command="UPDATE quiz_themes SET is_active=0 WHERE theme_key='your-theme-key'"
```
