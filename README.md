...
├── wrangler.test.toml              # Test-specific config (no DO self-reference)
...
npx wrangler d1 execute quizzer --local --file=./migrations/001_create_questions_table.sql
npx wrangler d1 execute quizzer --local --file=./migrations/002_seed_questions.sql
npx wrangler d1 execute quizzer --local --file=./migrations/003_create_hall_of_fame.sql
npx wrangler d1 execute quizzer --local --file=./migrations/004_add_quiz_themes.sql
npx wrangler d1 execute quizzer --local --file=./migrations/005_seed_quiz_themes.sql
npx wrangler d1 execute quizzer --local --file=./migrations/006_create_multiplayer_tables.sql
...
Unit tests use `wrangler.test.toml` instead of `wrangler.toml` because the Durable Object binding with `script_name` (self-reference) doesn't work in Vitest's test runner. The test config removes `script_name` to make DOs local to the test worker.
