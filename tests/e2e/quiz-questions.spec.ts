...
test.describe('Quiz Questions', () => {
    ...
    test('should display question information correctly', async ({ page }) => {
        ...
        await expect(questionCounter).toContainText('Question')

        const category = page.locator('.tag.category')
    ...
    test('should navigate between questions', async ({ page }) => {
        ...
        const questionCounter = page.locator('.progress-label')
        await expect(questionCounter).toContainText('Question')
        await expect(questionCounter).toContainText('2')
    })

    test('should show progress bar', async ({ page }) => {
        const progressBar = page.locator('.progress-track')
