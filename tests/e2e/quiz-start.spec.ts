...
test.describe('Quiz Start Screen', () => {
    ...
    test('should display the quiz title and subtitle', async ({ page }) => {
        ...
        await expect(page.locator('.hero-subtitle')).toContainText(
            'The ultimate quiz night experience'
        )
    })
    ...
    test('should display welcome message and available quiz modes', async ({
        ...
        await expect(individualMode).toContainText('Challenge yourself')
        ...
        const networkedMode = page.locator(
            '.mode-card:has-text("Online Battle")'
        )
        await expect(networkedMode).toBeVisible()
    })
    ...
    test('should navigate to quiz when starting solo mode', async ({
        ...
        await expect(page.locator('.number-selector')).toBeVisible()
        await expect(page.locator('.difficulty-selector')).toBeVisible()
        await expect(page.locator('.category-grid')).toBeVisible()

        // Click a category to start the quiz (clicking 'All Categories')
    ...
    test('should handle API errors gracefully', async ({ page, context }) => {
        ...
        // Click a category to trigger the API call
        await page.click('.category-card:has-text("All Categories")')
